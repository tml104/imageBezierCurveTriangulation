/*
    image triangulation for const color (prototype)
*/
    ...
    for (auto& triPtr : tmpTriList) {
        for (auto& coordinatePair : triPtr->pixelsList)
        {
            ...
            for (uint channelId = 0; channelId < 3; channelId++) {
                ...
                energySum += std::pow((hx - triPtr->constColor[channelId] / 256.0), 2);
            }
        }
    }
    ...
    for (auto& triPtr : tmpTriList) {
        for (auto& coordinatePair : triPtr->pixelsList)
        {
            ...
            for (uint channelId = 0; channelId < 3; channelId++) {
                ...
                double hx = imageInput.at<cv::Vec3b>(y, x)[channelId] / 256.0;
                energySum += std::pow((hx - triPtr->constColor[channelId] / 256.0), 2); // oh shit
            }
...
int main2()
{
    String imgPath = "./applefruit.jpg"; //applefruit 156 470
    
    /*
    * rmse:0.104059
    success_cnt_controlPoint:285 188 127 84 60
    success_cnt_vertex:7 8 5 4 1
    */
