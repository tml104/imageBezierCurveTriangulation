/*
    image triangulation for linear color
*/
...
struct TriangulationMesh {
    struct CurvedTriangle {
        ...
        cv::Vec3b getInterpolationResult(cv::Point2d p) {
            ...
            double ch1 = (aaa[0] * x + bbb[0] * y + ccc[0]); //bad implementation...
            double ch2 = (aaa[1] * x + bbb[1] * y + ccc[1]);
            double ch3 = (aaa[2] * x + bbb[2] * y + ccc[2]);
            ...
            // [debug]
            //std::cout << "[getInterpolationResult] ch123:" << ch1 << " " << ch2 << " " << ch3 << " " << std::endl;

            return { uchar(ch1), uchar(ch2), uchar(ch3) };
        }
        ...
        void solve(const cv::Mat& imageInput) {
            ...
            double n_A33 = pixelsList.size() * 1.0;
            ...
            Eigen::Vector3d B[3], X;

            auto ALU = A.lu();
            auto detVal = ALU.determinant();

            if (fabs(detVal) < 1e-12 || pixelsList.size()<=1) {
                // to const
                constFlag = true;
                ...
                // update b
                for (auto& coorPair : pixelsList) {
                    auto xb = coorPair.first;
                    auto yb = coorPair.second;
                    double xd = static_cast<double>(coorPair.first);
                    double yd = static_cast<double>(coorPair.second);


                    for (uint channelId = 0; channelId < 3; channelId++) {
                        double hxd = static_cast<double>(imageInput.at<cv::Vec3b>(yb, xb)[channelId]);

                        B[channelId](0) += xd * hxd;
                        B[channelId](1) += yd * hxd;
                        B[channelId](2) += hxd;
                    }
...
// fangda
void rasterization2(const cv::Mat& imageInput, TriangulationMesh& triangulationMesh, bool debug_flag, std::string imName) {
    ...
    cv::Mat imageResult = cv::Mat::zeros(imageInput.size()*SCALE_VALUE, CV_8UC3);
...
int main2()
{
    ...
    String imgPath = "./applefruit.jpg"; //applefruit
    /*
    159 470
    success_cnt_controlPoint:265 179 115 79 56
    success_cnt_vertex:6 9 7 6 4
    energySum:894.832 655.812 543.221 481.596 445.902 424.944
    rmse:0.13295 0.113817 0.103587 0.0975347 0.0938506 0.0916185
    */
